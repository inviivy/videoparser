...
H265SPS::H265SPS() { memset(reinterpret_cast<void *>(this), 0, sizeof(*this)); }
...
H265PPS::H265PPS() { memset(reinterpret_cast<void *>(this), 0, sizeof(*this)); }

H265VPS::H265VPS() { memset(reinterpret_cast<void *>(this), 0, sizeof(*this)); }
...
bool H265SliceHeader::IsISlice() const { return slice_type == kSliceTypeI; }

bool H265SliceHeader::IsPSlice() const { return slice_type == kSliceTypeP; }

bool H265SliceHeader::IsBSlice() const { return slice_type == kSliceTypeB; }
  ...
  if (pps->tiles_enabled_flag) {
    ...
    if (!pps->uniform_spacing_flag) {
      ...
      for (int i = 0; i < pps->num_tile_rows_minus1; ++i) {
        ...
        IN_RANGE_OR_RETURN(pps->row_height_minus1[i], 0,
                           pps->row_height_minus1[pps->num_tile_rows_minus1] -
                               1);
        pps->row_height_minus1[pps->num_tile_rows_minus1] -=
...
H265Parser::Result
H265Parser::ParseSliceHeaderForPictureParameterSets(const H265NALU &nalu,
                                                    int *pps_id) {
...
H265Parser::Result
H265Parser::ParseProfileTierLevel(bool profile_present,
...
H265Parser::Result
H265Parser::ParseScalingListData(H265ScalingListData *scaling_list_data) {
  for (int size_id = 0; size_id < 4; ++size_id) {
    for (int matrix_id = 0; matrix_id < 6;
         matrix_id += (size_id == 3) ? 3 : 1) {
...
H265Parser::Result
H265Parser::ParseAndIgnoreHrdParameters(bool common_inf_present_flag,
                                        int max_num_sub_layers_minus1) {
  Result res = kOk;
  int data;
...
H265Parser::Result H265Parser::ParseAndIgnoreSubLayerHrdParameters(
    int cpb_cnt, bool sub_pic_hrd_params_present_flag) {
  int data;
...
H265Parser::Result H265Parser::ParseRefPicListsModifications(
    const H265SliceHeader &shdr, H265RefPicListsModifications *rpl_mod) {
...
H265Parser::Result
H265Parser::ParsePredWeightTable(const H265SPS &sps,
  ...
  do {
    ...
    case H265SEIMessage::kSEIContentLightLevelInfo:
      READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(
          ...
          &num_bits_remain);
      READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(
          16, &sei_msg.content_light_level_info.max_picture_average_light_level,
          &num_bits_remain);
      break;
