...
private:
  ...
  Result ParseStRefPicSet(int st_rps_idx, const H265SPS &sps,
                          H265StRefPicSet *st_ref_pic_set,
  ...
  Result ParseAndIgnoreHrdParameters(bool common_inf_present_flag,
                                     int max_num_sub_layers_minus1);
  Result
  ParseAndIgnoreSubLayerHrdParameters(int cpb_cnt,
                                      bool sub_pic_hrd_params_present_flag);
  Result ParseRefPicListsModifications(const H265SliceHeader &shdr,
                                       H265RefPicListsModifications *rpl_mod);
  Result ParsePredWeightTable(const H265SPS &sps, const H265SliceHeader &shdr,
                              H265PredWeightTable *pred_weight_table);
  ...
  std::unordered_map<int, std::unique_ptr<H265VPS>> active_vps_;
  std::unordered_map<int, std::unique_ptr<H265SPS>> active_sps_;
  std::unordered_map<int, std::unique_ptr<H265PPS>> active_pps_;
};
